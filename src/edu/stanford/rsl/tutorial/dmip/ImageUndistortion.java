...
public class ImageUndistortion {
	...
	public void doImageUndstortion() {
		...
		// 1.Preprocessing //
		// ///////////////////
		// In the preprocessing part, an artificial distortion field is
		// generated.
		...
		int caseNo = 0;
		String filename = "/proj/i5dmip/zi74fuso/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/frame32.jpg";

		if (caseNo == 0) {
			filename = "/proj/i5dmip/zi74fuso/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/frame32.jpg";
		} else if (caseNo == 1) {
			filename = "/proj/i5dmip/zi74fuso/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/undistorted.jpg";
		} else if (caseNo == 2) {
			filename = "/proj/i5dmip/zi74fuso/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/frame90.jpg";
		}
		...
		for (int i = 0; i < image.getWidth(); i++) {
			for (int j = 0; j < image.getHeight(); j++) {
				float scaledValue = (image.getAtIndex(i, j) - min)
						/ (max - min);
		...
		for (int i = 0; i < R.getWidth(); i++) {
			for (int j = 0; j < R.getHeight(); j++) {
				float r = (float) (d * Math.sqrt(a
						* Math.pow(((X.getAtIndex(i, j) - half) / imSize), 2)
						+ b
						* Math.pow(((Y.getAtIndex(i, j) - half) / imSize), 2)));
				R.setAtIndex(i, j, r);
			}
		}
		...
		// distorted image coordinates = undistorted image points + artificial
		// distortion field
		...
		// Resample the original input image at the distorted image points
		// (XD,YD) to create artificial distortion

		Grid2D distortedImage = new Grid2D(imSize, imSize);
		for (int i = 0; i < R.getWidth(); i++) {
			for (int j = 0; j < R.getHeight(); j++) {
				float distortedValue = InterpolationOperators
						.interpolateLinear(image, Xd.getAtIndex(i, j),
								Yd.getAtIndex(i, j));
		...
		// Here, the distorted and undistorted coordinates from the
		// preprocessing
		// can be used.
		...
		for (int i = 0; i < ny; i++) {
			for (int j = 0; j < nx; j++) {
				// TODO: sample the distorted and undistorted grid points at the
				// lattice points
				// TODO
				// TODO
				// TODO
		...
		// Compute the distorted points: be aware of the fact, that the
		// artificial deformation takes
		...
		// d=1: oblique line with y-intercept a_0 & slope a_1 -> f(x)=a_0 + a_1
		// x
		// d=2: parabola
		...
		// Compute the pseudo-inverse of A with the help of the SVD (class:
		// DecompositionSVD)
		// TODO
		// TODO
		...
		// 4. Compute the distorted grid points (xDist, yDist) which are used to
		// sample the
				...
				for (int k = 0; k <= degree; k++) {
					for (int l = 0; l <= degree - k; l++) {
						xDist.setAtIndex(
								x,
								y,
								(float) (xDist.getAtIndex(x, y) + u_vec
										.getElement(cc)
										* Math.pow(x, k)
										* Math.pow(y, l)));
						yDist.setAtIndex(
								x,
								y,
								(float) (yDist.getAtIndex(x, y) + v_vec
										.getElement(cc)
										* Math.pow(x, k)
										* Math.pow(y, l)));
						cc++;
		...
		Grid2D differenceImage = (Grid2D) NumericPointwiseOperators
				.subtractedBy(quadraticImage, undistortedImage);
