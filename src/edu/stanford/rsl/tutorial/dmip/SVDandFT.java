...
public class SVDandFT {
	...
	public static void invertSVD(SimpleMatrix A) {
		...
		SimpleMatrix temp = SimpleOperators.multiplyMatrixProd(svd.getU(),
				svd.getS());
		SimpleMatrix A2 = SimpleOperators.multiplyMatrixProd(temp, svd.getV()
				.transposed());
		...
		SimpleMatrix Sinv = new SimpleMatrix(svd.getS().getRows(), svd.getS()
				.getCols());
		Sinv.zeros();
		...
		SimpleMatrix tempInv = SimpleOperators.multiplyMatrixProd(svd.getV(),
				svd.getreciprocalS());
		SimpleMatrix Ainv = SimpleOperators.multiplyMatrixProd(tempInv, svd
				.getU().transposed());
		System.out.println("Ainv        = " + Ainv.toString());
		System.out.println("A.inverse() = "
				+ A.inverse(InversionType.INVERT_SVD));
		...
		SimpleMatrix templowrank = SimpleOperators.multiplyMatrixProd(
				svd.getU(), Slowrank);
		...
		// if we set vector b to the vector consisting of 1's, we imply a 0.1%
		// change
	...
	public static void optimizationProblem1(SimpleMatrix A, int rankDeficiency) {
		...
		// p = min(m, n). Of course, in practice A does not fulfill this
		// constraint.
		...
		// to have a rank deficiency of one, and the two non-zero singular
		// values
		...
		SimpleMatrix Slowrank = new SimpleMatrix(svd.getS().getRows(), svd
				.getS().getCols());
		Slowrank.zeros();
		// Fill in remaining singular values with the mean.
		for (int i = 0; i < newRank; i++) {
			Slowrank.addToElement(i, i, mean);
		}
		// compute A0
		SimpleMatrix temp = SimpleOperators.multiplyMatrixProd(svd.getU(),
				Slowrank);
		SimpleMatrix A0 = SimpleOperators.multiplyMatrixProd(temp, svd.getV()
				.transposed());
	...
	public static void optimizationProblem4(double[] xCoords, double[] yCoords) {
		...
		// y = A*b
		// Note: y = m*x +t*1 Thus: A contains (x_i 1) in each row, and y
		// contains the y_i coordinates
		...
		// Note: We need to use SVD matrix inversion because the matrix is not
		// square
		// more samples (7) than unknowns (2) -> overdetermined system ->
		// least-square
		// solution.
	...
	public static void optimizationProblem2(SimpleMatrix b) {
		...
		// The objective function is linear in the components of A, thus the
		// whole
		// sum can be rewritten in matrix notation:
		// Ma = 0
		// where the measurement matrix M is built from single elements of the
		// sum.
		// Given the four points we get four equations and therefore four rows
		// in M:
		...
		SimpleVector a = svd.getV().getCol(svd.getV().getCols() - 1);
	...
	public static void optimizationProblem3(Grid2D image, int rank) {
		...
		// In order to apply the svd, we first need to transfer our Grid2D image
		// to a matrix
		...
		// output images
		Grid3D imageRanks = new Grid3D(image.getWidth(), image.getHeight(),
				rank);

		// Create Rank k approximations
		for (int k = 0; k < rank; k++) {

			SimpleVector us = svd.getU().getCol(k)
					.multipliedBy(svd.getSingularValues()[k]);
			SimpleMatrix Iapprox = SimpleOperators.multiplyOuterProd(us, svd
					.getV().getCol(k));
			...
			for (int i = 0; i < image.getHeight(); i++) {
				for (int j = 0; j < image.getWidth(); j++) {
					if (k == 0) {
						imageRank.setAtIndex(j, i,
								(float) Iapprox.getElement(i, j));
					} else {
						imageRank.setAtIndex(j, i,
								(float) Iapprox.getElement(i, j));
						imageRank.addAtIndex(j, i,
								imageRanks.getAtIndex(j, i, k - 1));
					}
				}
			}
		...
		SimpleMatrix us = SimpleOperators.multiplyMatrixProd(svd.getU()
				.getSubMatrix(0, 0, svd.getU().getRows(), rank), svd.getS()
				.getSubMatrix(0, 0, rank, rank));
		SimpleMatrix IapproxK = SimpleOperators.multiplyMatrixProd(us, svd
				.getV().getSubMatrix(0, 0, svd.getV().getRows(), rank)
				.transposed());
	...
	public static void fourierExercise(Grid2D image) {
		...
		// Important: Grid2DComplex enlarges the original image to the next
		// power of 2
		// When transforming back, make sure to prune the image size to your
		// original image
	...
	public static void main(String[] args) {
		...
		// 2. In the Pipeline window, go to edit configuration and press "center
		// volume" and save
		// 3. In the ImageJ window, navigate to Plugins->CONRAD->Create
		// Numerical Phantom
		...
		Grid3D sheppLoganVolume = ImageUtil.wrapImagePlus(IJ
				.openImage(filenameShepp));
