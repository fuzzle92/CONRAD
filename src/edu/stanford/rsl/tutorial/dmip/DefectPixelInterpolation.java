...
public class DefectPixelInterpolation {
	 ...
	 * 
	 * @param image
	 *            corrupted image with defect pixels
	 * @param mask
	 *            binary image mask of the defect pixels (0: defect, 1: fine)
	 * @param maxIter
	 *            maximum number of iterations
	 * @param zeroPadding
	 *            enables zero padding for the input images. Images are enlarged
	 *            to size of next power of 2 and filled with zeros
	 * @return image, where defect pixels are interpolated
	 */
	public Grid2D interpolateSpectral(Grid2D image, Grid2D mask, int maxIter,
			boolean zeroPadding) {
		// padding
		...
		Grid3D visualize = new Grid3D(image.getWidth(), image.getHeight(),
				maxIter / 100);
		...
		for (int i = 0; i < maxIter; i++) {
			...
			// which maximizes the energy reduction [Paragraph after Eq. (16) in
			// the paper]
			double maxDeltaE_G = Double.NEGATIVE_INFINITY;
			// create arraylist to store lines (in case multiple maxima are
			// found)
			// TODO
			...
			// if there were more indices than one with the same max_value, pick
			// a random one of these
			int idx = (int) Math.floor(Math.random() * sj1.size());
			int s1 = sj1.get(idx)[0];
			int t1 = sj1.get(idx)[1];

			// Calculate the ratio of energy reduction in comparison to the last
			// iteration
			if (i > 0) {
				maxDeltaE_G_Ratio = Math.abs(maxDeltaE_G - lastEredVal
						/ maxDeltaE_G);
			}
			...
			if ((s1 == 0 && t1 == 0) || (s1 == halfDim[0] && t1 == 0)
					|| (s1 == 0 && t1 == halfDim[1])
			...
			if (i % 100 == 0) {
				...
				// Fill in the defect mask pixels with current estimation and
		...
		// Fill in the defect mask pixels with the current estimation and remove
		// the zero padding
		...
		for (int i = 0; i < result.getWidth(); i++) {
			for (int j = 0; j < result.getHeight(); j++) {
				if (mask.getAtIndex(i, j) == 0) {
					result.setAtIndex(i, j, FHatNext.getRealAtIndex(i, j));
				}
			}
		}
	...
	/**
	 * 
	 * Do the convolution of the m-times-n matrix F and W s,t is the position of
	 * the selected line pair, the convolution is simplified in the following
	 * way: G(k1,k2) = F(k1,k2) 'conv' W(k1,k2) = (F(s,t)W(k1-s,k2-t) +
	 * F*(s,t)W(k1+s,k2+t)) / (MN) where F* is the conjugate complex.
	 *
	 * @param G
	 *            Fourier transformation of input image
	 * @param FHatNext
	 *            currently estimated FT of the fixed image
	 * @param FHat
	 *            previous estimated FT of the fixed image
	 * @param W
	 *            Fourier transformation of the mask image
	 * @param s1
	 *            position of the selected line pair
	 * @param t1
	 *            position of the selected line pair
	 * @param specialCase
	 */
	private static void updateErrorSpectrum(Grid2DComplex G,
			Grid2DComplex FHatNext, Grid2DComplex FHat, Grid2DComplex W,
			int s1, int t1, boolean specialCase) {
		int[] sz = FHatNext.getSize();

		// Accumulation: Update pair (s1,t1),(s2,t2)
		Complex F_st = new Complex(FHatNext.getRealAtIndex(s1, t1)
				- FHat.getRealAtIndex(s1, t1), FHatNext.getImagAtIndex(s1, t1)
				- FHat.getImagAtIndex(s1, t1));
		...
		for (int j = 0; j < sz[1]; j++) {
			for (int i = 0; i < sz[0]; i++) {
				...
				if (specialCase) {
					...
					GVal = new Complex(G.getRealAtIndex(i, j),
							G.getImagAtIndex(i, j));
					Complex WNeg = new Complex(W.getRealAtIndex(xneg, yneg),
							W.getImagAtIndex(xneg, yneg));
					...
					Complex WPos = new Complex(W.getRealAtIndex(xpos, ypos),
							W.getImagAtIndex(xpos, ypos));
					Complex WNeg = new Complex(W.getRealAtIndex(xneg, yneg),
							W.getImagAtIndex(xneg, yneg));
					GVal = new Complex(G.getRealAtIndex(i, j),
							G.getImagAtIndex(i, j));
					GVal = GVal.sub(((F_st.mul(WNeg)).add(F_st_conj.mul(WPos)))
							.div(MN));
	...
	public Grid2D interpolateMedian(Grid2D image, Grid2D defects,
			int kernelWidth, int kernelHeight) {
		// Pad the image. Otherwise, the filter will ignore kernelWidth/2 at
		// each side of the image
		Grid2D paddedImage = new Grid2D(image.getWidth() + kernelWidth,
				image.getHeight() + kernelHeight);

		for (int i = 0; i < image.getWidth(); i++) {
			for (int j = 0; j < image.getHeight(); j++) {
				float val = image.getAtIndex(i, j);
				paddedImage.setAtIndex(i + kernelWidth / 2, j + kernelHeight
						/ 2, val);
			}
		}
		paddedImage.show();
			...
			for (int j = 0; j < image.getHeight(); j++) {
				if (defects.getAtIndex(i, j) == 0) {
					...
					result.setAtIndex(
							i,
							j,
							medianFiltered.getAtIndex(i + kernelWidth / 2, j
									+ kernelHeight / 2));
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		String filename = "/proj/i5dmip/zi74fuso/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/testimg.bmp";
		String filenameMask = "/proj/i5dmip/zi74fuso/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/mask.bmp";

		Grid2D image = ImageUtil.wrapImagePlus(IJ.openImage(filename))
				.getSubGrid(0);
		image.show("Ideal Input Image");

		Grid2D mask = ImageUtil.wrapImagePlus(IJ.openImage(filenameMask))
				.getSubGrid(0);
		// Set some pixels as defect, elementwise multiply with defect pixel
		// mask
		...
		Grid2D medianFiltered = dpi.interpolateMedian(defectImage, mask,
		...
		for (int i = 0; i < absDiffMedian.getWidth(); i++) {
			for (int j = 0; j < absDiffMedian.getHeight(); j++) {
				float val = Math.abs(medianFiltered.getAtIndex(i, j)
						- image.getAtIndex(i, j));
		...
		// TODO
		Grid2D spectralFiltered = dpi.interpolateSpectral(defectImage, mask,
				maxIter, zeroPadding);
		...
		for (int i = 0; i < absDiffSpectral.getWidth(); i++) {
			for (int j = 0; j < absDiffSpectral.getHeight(); j++) {
				float val = Math.abs(spectralFiltered.getAtIndex(i, j)
						- image.getAtIndex(i, j));
